 ...
 * Author: pablo
 *
 * Created on 17 de julio de 2023, 11:28 p.m.
 */
...
setup();
    while(1){
        if (PORTBbits.RB0 == 1) {
            PORTA++;
            while (PORTBbits.RB0 == 1) {    //While antirebote
                ;
            }
        }if (PORTBbits.RB1 == 1) {
            PORTA--;
            while (PORTBbits.RB1 == 1) {    //While antirebote
                ;
            }
        }
    }
...
void setup(void){
    ...
    TRISB = 1;  //Puerto B como entradas
    PORTA = 0;  //Se inicializa el puerto A en 0 todos.
    PORTB = 0;  //Se inicializa el puerto B en 0 todos.
}